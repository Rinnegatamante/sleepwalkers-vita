/* main.c -- Sleepwalker's Journey .so loader
 *
...
void patch_game(void) {
	...
	hook_addr((uintptr_t)funky_mod.text_base + 0x38D6F4, (uintptr_t)_sync_synchronize);
...
void glShaderSource_hook(GLuint shader, GLsizei count, const GLchar **string, const GLint *length) {
	...
	//printf("Shader: %s\n", sha_name);
	if (!file) {
		snprintf(cg_path, sizeof(cg_path), "ux0:data/sleepwalkers/%s.glsl", sha_name);
